...
- `src/components/` – Data-only component resources such as `StatsComponent`, `JobComponent`, `TraitComponent`, and `Trait` that encode modular attributes for EntityData manifests.【F:src/components/StatsComponent.gd†L1-L26】【F:src/components/JobComponent.gd†L1-L40】【F:src/components/TraitComponent.gd†L1-L38】【F:src/components/Trait.gd†L1-L19】 The directory now also hosts combat-facing resources like `CombatRuntimeComponent` (tracks initiative rolls, turn-based modifiers, and round resets) and `FactionComponent` (records team allegiance and reputation), keeping tactical systems fully data-driven.【F:src/components/CombatRuntimeComponent.gd†L1-L46】【F:src/components/FactionComponent.gd†L1-L32】
...
- `src/systems/` – Reusable gameplay systems such as `DebugSystem` that operate on grouped entities through the EventBus pattern.【F:src/systems/DebugSystem.gd†L1-L118】 The combat subdirectory introduces `CombatTimer`, which orchestrates encounter setup, initiative queues, and turn advancement using the shared `CombatEncounterState` resource so downstream combat logic only responds to EventBus broadcasts.【F:src/systems/combat/CombatTimer.gd†L1-L260】【F:src/core/CombatEncounterState.gd†L1-L60】
