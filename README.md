...
- `src/components/` – Data-only component resources such as `StatsComponent` that encode modular attributes for EntityData manifests.【F:src/components/StatsComponent.gd†L1-L26】
- `src/systems/` – Reusable gameplay systems such as `DebugSystem` and `TraitComponent` that operate on grouped entities through the EventBus pattern.【F:src/systems/DebugSystem.gd†L1-L118】【F:src/systems/TraitComponent.gd†L1-L36】 
