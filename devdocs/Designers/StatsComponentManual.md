...
Job resources expose designer-friendly arrays so profession data can be composed without touching dictionaries. Use the `stat_bonuses` and `training_bonuses` lists to add any number of `JobStatBonus` or `JobTrainingBonus` entries, each pairing a StatsComponent property with an additive modifier. `starting_skills` accepts direct references to `Skill` resources, and `starting_traits` links `Trait` resources that should be injected when the job is active.【F:assets/jobs/Job.gd†L1-L88】【F:assets/jobs/MageJob.gd†L1-L63】
